...
## Diffusion models 
(this code located in the folder `shared/jet_interpretability_diffusion/GenerativeModelsOnPhaseSpace-main` on Killarney)
...
- Train a score network using `scripts/train_lightning.py` or `slurm_scripts/train.sh`. Right now, the diffusion schedule options need to be set by hand rather than pulling from the pre-computed events; this should be fixed ASAP so there is never any confusion.
...
- Check that the trained model looks good by generating some events using `scripts/generate_samples.py` or `slurm_scripts/generate.sh`.
...
- Generate "round-trip" events with `scripts/forward_backward_sample.py` or `slurm_scripts/generate_fb_roundtrip.sh`. This script takes as input the q-space training data `Q0.pt`, applies the forward diffusion process for `T_FORWARD` steps to the first `N_SAMPLES` events for `N_TRAJ` independent trajectories, then uses the trained score model to denoise back to t = 0.
